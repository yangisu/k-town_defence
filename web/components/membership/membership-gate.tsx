...
export function MembershipGate({ children }: { children: ReactNode }) {
  const { status, fandoms, error, isSelecting, selectFandom, retry } = useMembership();
  ...
  }, [needsSignIn]); const [selected, setSelected] = useState(""); const [invalid, setInvalid] = useState(false); const first = useRef<HTMLInputElement>(null);
