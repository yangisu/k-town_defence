...
export function ExpeditionView({ expeditionId, services, onStartCheckIn, onBack }: { expeditionId: string | null; services: AppServices; onStartCheckIn: (placeId: string) => void; onBack: () => void }) {
  ...
  const ordered = expedition.stopIds.map((id) => places.find((place) => place.id === id)).filter(Boolean) as Place[];
  return (
    <div className="view expedition-view">
      ...
      <div className="expedition-layout"><section className="itinerary-panel"><div className="section-heading"><div><span className="eyebrow">TODAY&apos;S ROUTE</span><h2>오늘의 원정</h2></div><strong>{expedition.completedStops} / {expedition.stopIds.length} 완료</strong></div><div className="route-progress"><i style={{ width: `${(expedition.completedStops / expedition.stopIds.length) * 100}%` }} /></div><ol className="stop-list">{ordered.map((place, index) => { const done = index < expedition.completedStops - 1; const current = index === expedition.completedStops - 1; return <li key={place.id} className={current ? "current" : done ? "done" : ""}><div className="stop-marker">{done ? <Check size={16} /> : String(index + 1).padStart(2, "0")}</div><div className="stop-copy"><span>{place.categoryLabel}</span><h3>{place.nameKo}</h3><p>{place.description}</p><div className="stop-meta"><span><MapPin size={13} /> {place.transit}</span><span><Footprints size={13} /> 체류 {place.dwellMinutes}분</span>{place.localBenefit ? <span className="benefit">{place.localBenefit}</span> : null}</div></div><div className="stop-action"><strong>+{place.points}P</strong>{current ? <button onClick={() => onStartCheckIn(place.id)} aria-label={`${place.nameKo} 체크인`}>체크인</button> : null}</div></li>; })}</ol></section>
