...
- 격리 DB: 실행 시 새로 생성한 `ktown_ac1_retry`
...
- 빈 DB migration 검증: `python -m alembic upgrade head`가 `20260921_0012`까지
  적용된다 (exit 0).
- 전체 검증: `python -m pytest -q`가 신규 BTS 경로·migration·legacy 회귀를
  포함해 통과한다 (`208 passed, 196 subtests passed`, exit 0).
