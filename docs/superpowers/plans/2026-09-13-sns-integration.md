...
> **Scope change (post-implementation):** Naver login was implemented and verified working end-to-end (see Task 1's Step 5 notes), then removed by request. `SocialProvider` is now `"kakao" | "google"` only; `web/lib/server/social-auth.ts` no longer has a `naver` entry, `NAVER_CLIENT_ID`/`NAVER_CLIENT_SECRET` are gone from both `.env.example` files, and the Naver-specific test case was deleted from `web/tests/social-auth.test.ts`. The rest of this document still narrates Naver in the original three-provider design — read it as history, not current scope.
