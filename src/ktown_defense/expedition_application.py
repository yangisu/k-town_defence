...
class ExpeditionApplication:
    ...
    async def create(
        ...
        *,
        ...
        if active is not None:
            if active.recommendation_id == recommendation_id:
                if route_key == "bts-busan":
                    stored = {
                        stop.place_id for stop, _ in await self.stops(active.id)
                        if stop.stop_kind == "recommendation"
                    }
                    if stored != set(selected_recommendation_place_ids or []):
                        raise ApiError(409, "ACTIVE_EXPEDITION_EXISTS", "진행 중인 원정의 선택 장소가 다릅니다.")
                return active
        ...
        selected_ids = set(selected_recommendation_place_ids or [])
        recommendation_ids = {
            stop.candidate.id for stop in recommendation.stops if stop.kind == "recommendation"
        }
        anchor_ids = {stop.candidate.id for stop in recommendation.stops if stop.kind == "anchor"}
        if route_key == "bts-busan" and (
            not selected_ids <= recommendation_ids or bool(selected_ids & anchor_ids)
        ):
            raise ApiError(409, "EXPEDITION_SELECTION_INVALID", "선택한 추천 장소가 현재 경로에 없습니다.")
        persisted_stops = [
            stop for stop in recommendation.stops
            if stop.required or route_key != "bts-busan" or stop.candidate.id in selected_ids
        ]
        ...
        for order, stop in enumerate(persisted_stops, start=1):
            self._session.add(ExpeditionStopModel(
                ...
                recommendation_source=(
                    str(stop.evidence.get("source")) if stop.evidence else None
                ),
                recommendation_reason=(
                    str(stop.evidence.get("reason")) if stop.evidence else None
                ),
                recommendation_metadata=stop.evidence or {},
            ))
