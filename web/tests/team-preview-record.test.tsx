...
function completedEnglishSession() {
  ...
  return demoSessionReducer(selected, { type: "completeMission", missionId: "busan-1", award: missionAward });
}
...
it("clears preview selection and history without changing the selected locale", () => {
  ...
  expect(reset.completedMissionIds).toEqual([]);
  expect(reset.missionHistory).toEqual([]);
