...
export function MembershipGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { if (tutorial && status === "selection_required" && tutorial.state.status === "idle") tutorial.startTutorial(); }, [status, tutorial]);
  ...
  if (status === "error") return <main className="membership-gate"><section className="membership-card"><h1>{copy[error?.code ?? "UNKNOWN_ERROR"] ?? copy.UNKNOWN_ERROR}</h1>{error?.code === "AUTHENTICATION_REQUIRED" ? <a className="primary-button" href="/signin?return_to=%2F">로그인하기</a> : <button onClick={() => void retry()}>다시 시도</button>}</section></main>;
  if (status === "ready") return children;
  const submit = (event: FormEvent) => { event.preventDefault(); if (!selected) { setInvalid(true); first.current?.focus(); return; } void selectFandom(selected).then(() => tutorial?.completeStep("choose-fandom")); };
  return <><main className="membership-gate"><form className="membership-card" onSubmit={submit}><span className="eyebrow">SEASON MEMBERSHIP</span><h1>함께 여행할 팬덤을 선택해 주세요</h1><p>부산 탐험 기록이 선택한 팬덤에 연결돼요.</p><fieldset disabled={isSelecting}><legend className="sr-only">팬덤 선택</legend><div className="fandom-options">{fandoms.map((fandom, index) => <FandomOption key={fandom.id} ref={index ? undefined : first} fandom={fandom} checked={selected === fandom.id} disabled={isSelecting} onChange={() => { setSelected(fandom.id); setInvalid(false); }} />)}</div></fieldset>{invalid && <p role="alert">팬덤을 선택해 주세요</p>}{error && <p role="alert">{copy[error.code] ?? copy.UNKNOWN_ERROR}</p>}{isSelecting && <p role="status">팬덤을 저장하고 있어요</p>}<button data-tutorial="choose-fandom" className="primary-button" disabled={isSelecting}>이 팬덤으로 시즌 시작</button></form></main><TutorialOverlay locale="ko" /></>;
}
