...
it("traps the artist drawer, closes it with Escape, and returns focus to its trigger", async () => {
  ...
  const trigger = await screen.findByRole("button", { name: "아티스트 선택" });
...
it("renders all stronghold states as distinct named silhouettes with visible labels", () => {
  ...
  expect(screen.getByRole("img", { name: "씨앗 거점" })).toHaveTextContent("씨앗");
  expect(screen.getByRole("img", { name: "나무 거점" })).toHaveTextContent("나무");
  expect(screen.getByRole("img", { name: "랜드마크 거점" })).toHaveTextContent("랜드마크");
});
